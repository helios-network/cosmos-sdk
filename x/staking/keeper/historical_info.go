...
func (k Keeper) TrackHistoricalInfo(ctx context.Context) error {
	...
	inactiveVals, err := k.GetInactiveValidators(ctx)
	if err != nil {
		return err
	}
	...
	for i, val := range activeVals {
		...
		if err == nil {
			...
			// MIGRATION: Ensure validator cache is updated in store when optimization activates
			if useOptimized {
				k.SetValidator(ctx, activeVals[i])
			}
		}
	}
	...
	for i, val := range inactiveVals {
		...
		if err == nil {
			...
			// MIGRATION: Ensure validator cache is updated in store when optimization activates
			if useOptimized {
				k.SetValidator(ctx, inactiveVals[i])
			}
		}
	}
