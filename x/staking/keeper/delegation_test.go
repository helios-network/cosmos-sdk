...
func (s *KeeperTestSuite) TestUndelegateSelfDelegationBelowMinSelfDelegation() {
	...
	_, _, err := keeper.Undelegate(ctx, val0AccAddr, addrVals[0], math.LegacyNewDecFromInt(keeper.TokensFromConsensusPower(ctx, 6)), sdk.DefaultBondDenom)
	require.NoError(err)
...
func (s *KeeperTestSuite) TestUndelegateFromUnbondingValidator() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromModuleToModule(gomock.Any(), stakingtypes.BondedPoolName, stakingtypes.NotBondedPoolName, gomock.Any())
	_, amount, err := keeper.Undelegate(ctx, val0AccAddr, addrVals[0], math.LegacyNewDecFromInt(delTokens), sdk.DefaultBondDenom)
	require.NoError(err)
	...
	_, undelegatedAmount, err := keeper.Undelegate(ctx, addrDels[1], addrVals[0], undelegateAmount, sdk.DefaultBondDenom)
	require.NoError(err)
...
func (s *KeeperTestSuite) TestUndelegateFromUnbondedValidator() {
	...
	// unbond the all self-delegation to put validator in unbonding state
	s.bankKeeper.EXPECT().SendCoinsFromModuleToModule(gomock.Any(), stakingtypes.BondedPoolName, stakingtypes.NotBondedPoolName, gomock.Any())
	_, amount, err := keeper.Undelegate(ctx, val0AccAddr, addrVals[0], math.LegacyNewDecFromInt(valTokens), sdk.DefaultBondDenom)
	require.NoError(err)
	...
	_, amount2, err := keeper.Undelegate(ctx, addrDels[1], addrVals[0], math.LegacyNewDecFromInt(unbondTokens), sdk.DefaultBondDenom)
	require.NoError(err)
	require.Equal(amount2, unbondTokens)

	// unbond rest of the other delegation's shares
	remainingTokens := delTokens.Sub(unbondTokens)
	_, amount3, err := keeper.Undelegate(ctx, addrDels[1], addrVals[0], math.LegacyNewDecFromInt(remainingTokens), sdk.DefaultBondDenom)
	require.NoError(err)
...
func (s *KeeperTestSuite) TestUnbondingAllDelegationFromValidator() {
	...
	_, amount2, err := keeper.Undelegate(ctx, addrDels[1], addrVals[0], math.LegacyNewDecFromInt(delTokens), sdk.DefaultBondDenom)
	require.NoError(err)
...
func (s *KeeperTestSuite) TestRedelegateToSameValidator() {
	...
	_, err := keeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[0], math.LegacyNewDec(5), sdk.DefaultBondDenom)
	require.Error(err)
}
...
func (s *KeeperTestSuite) TestRedelegationMaxEntries() {
	...
	for i := uint32(0); i < maxEntries; i++ {
		var err error
		completionTime, err = keeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[1], math.LegacyNewDec(1), sdk.DefaultBondDenom)
		require.NoError(err)
	}

	// an additional redelegation should fail due to max entries
	_, err = keeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[1], math.LegacyNewDec(1), sdk.DefaultBondDenom)
	require.Error(err)
...
func (s *KeeperTestSuite) TestRedelegateSelfDelegation() {
	...
	_, err := keeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[1], math.LegacyNewDecFromInt(delTokens), sdk.DefaultBondDenom)
	require.NoError(err)
...
func (s *KeeperTestSuite) TestRedelegateFromUnbondingValidator() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromModuleToModule(gomock.Any(), stakingtypes.NotBondedPoolName, stakingtypes.BondedPoolName, gomock.Any())
	_, err = keeper.BeginRedelegation(ctx, addrDels[1], addrVals[0], addrVals[1], math.LegacyNewDecFromInt(redelegateTokens), sdk.DefaultBondDenom)
	require.NoError(err)
...
func (s *KeeperTestSuite) TestRedelegateFromUnbondedValidator() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromModuleToModule(gomock.Any(), stakingtypes.NotBondedPoolName, stakingtypes.BondedPoolName, gomock.Any())
	_, err = keeper.BeginRedelegation(ctx, addrDels[1], addrVals[0], addrVals[1], math.LegacyNewDecFromInt(redelegationTokens), sdk.DefaultBondDenom)
	require.NoError(err)
